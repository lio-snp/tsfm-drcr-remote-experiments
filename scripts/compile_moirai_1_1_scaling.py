#!/usr/bin/env python
"""Compile Moirai-1.1 paired scaling metrics and bootstrap interactions."""
...
MODEL_META = {
    "moirai_1_1_small": {"size_label": "small", "params_m": 14.0, "size_rank": 0},
    "moirai_1_1_base": {"size_label": "base", "params_m": 91.0, "size_rank": 1},
    "moirai_1_1_large": {"size_label": "large", "params_m": 311.0, "size_rank": 2},
}
...
def main() -> None:
    ...
    parser.add_argument("--metrics-glob", default="results/window_metrics/moirai_1_1_*_scaling_*_metrics.csv")
    ...
    combined = annotate_metric_rows(metric_paths, read_csv(manifest_path))
    ...
    bootstrap.update(
        {
            ...
            "framing": (
                "Tests whether the pretrained zero-shot size interaction seen in "
                "Chronos-Bolt transfers to a second clean size family, Moirai-1.1."
            ),
            "targets": sorted({str(row["target_key"]) for row in combined}),
            "models": sorted({str(row["model"]) for row in combined}),
            "generated_at": int(time.time()),
        }
    )

    write_csv(output_dir / "moirai_1_1_scaling_window_metrics.csv", combined)
    write_csv(output_dir / "moirai_1_1_scaling_summary.csv", summary)
    write_csv(output_dir / "moirai_1_1_scaling_slopes.csv", slopes)
    (output_dir / "moirai_1_1_scaling_bootstrap.json").write_text(json.dumps(bootstrap, indent=2))
    print(f"Wrote Moirai-1.1 scaling summary for {len(combined)} paired metric rows")
