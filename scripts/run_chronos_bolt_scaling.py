#!/usr/bin/env python
"""Run the paired Chronos-Bolt capacity scaling experiment."""
...
MODELS = {
    "tiny": {"model_id": "amazon/chronos-bolt-tiny", "model_name": "chronos_bolt_tiny"},
    "mini": {"model_id": "amazon/chronos-bolt-mini", "model_name": "chronos_bolt_mini"},
    "small": {"model_id": "amazon/chronos-bolt-small", "model_name": "chronos_bolt_small"},
    "base": {"model_id": "amazon/chronos-bolt-base", "model_name": "chronos_bolt_base"},
}
...
def main() -> None:
    ...
    parser.add_argument("--models", default="tiny,mini,small,base")
    ...
    for model_key in selected_models:
        ...
        for target_key in selected_targets:
            ...
            output_slug = (
                f"{model['model_name']}_scaling_{target['target_key']}_{target['baseline_mode']}"
            )
    ...
    run(
        [
            py,
            "scripts/compile_chronos_bolt_scaling.py",
            "--manifest",
            args.manifest,
            "--bootstrap",
            str(args.bootstrap),
    ...
    status_path = ROOT / "results" / "scaling" / "chronos_bolt_scaling_run_status.json"
    ...
    print(f"Wrote scaling run status to {status_path}")
